    ...
    public partial class ProbeSetupPage : UserControl, INotifyPropertyChanged
    {
        ...
        private async Task LoadProbeDetailsFromInstallationDataAsync(string partNo)
        {
            ...
            try
            {
                ...
                const string query = @"SELECT ProbeId, Stroke FROM ProbeInstallationData WHERE PartNo = @PartNo AND Status = @Status";
                using var cmd = new SqlCommand(query, con);
                ...
                while (await reader.ReadAsync() && index < Probes.Count)
                {
                    ...
                    probe.ID = reader.GetString(0);
                    probe.Stroke = reader.GetString(1);
                    index++;
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error loading probe details: {ex.Message}", "Database Error", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < totalCount; i++)
            {
                try
                {
                    ...
                    if (double.TryParse(Probes[i].Stroke, out double strokeValue) && strokeValue > 0)
                    {
                        // Calculate percent progress relative to Stroke
                        double percentage = (reading / strokeValue) * 100;
                        Probes[i].Value = Math.Min(Math.Max(percentage, 0), 100);
                        Probes[i].InRange = true;
                    }
                    else
                    {
                        Probes[i].Value = 0;
        ...
        private async void StartBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    // Swallow any exception during disconnect
                }
            }
        }
